...
import authorization from "./app/services/authorizationService";
...
export default async function middleware(
  ...
  const token:string | undefined =cookies().get("user")?.value;
  const UserIsNotAuthenticated: boolean = !userRole || !userRole.length || !token || !token.length ;
